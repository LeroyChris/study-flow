...
export default function TimerSettingsModal({ isOpen, onClose, currentSettings, onSave }) {
  // bikin state local buat nyimpen angka durasi yang diketik sama user
  ...
  // kalo status modalnya gak kebuka (false), langsung cegah render apa-apa
  if (!isOpen) return null

  // fungsi pas user ngeklik tombol save, data dikirim balik ke parent component
  const handleSubmit = (e) => {
    e.preventDefault()
    // paksa angka input minimal 1 menit biar user gak bisa set waktu minus atau 0
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-800 rounded-[32px] w-full max-w-md p-8 shadow-xl mx-4 relative">
        ...
        {/* tombol silang kecil dipojok buat nutup modal tanpa nyimpen perubahan */}
        <button onClick={onClose} className="absolute top-6 right-6 text-gray-400 cursor-pointer text-xl">✕</button>
        
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6 tracking-tight">Timer Settings</h2>
        
        {/* form input angka durasi waktu pomodoro */}
