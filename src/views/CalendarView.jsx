          ...
          <div className="flex flex-col gap-4">
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden bg-white">
                ...
                {/* Grid Hari Kalender */}
                <div className="grid grid-cols-7 divide-x divide-y divide-gray-100 border-t border-l border-gray-100 bg-white">
                  {days.map((d, i) => {
                    ...
                    return (
                      <div
                        ...
                        className={`h-16 p-1 flex flex-col justify-between hover:bg-blue-50/30 transition-all cursor-pointer ${
                          isSelected ? 'bg-blue-50/60 ring-1 ring-blue-400 ring-inset' : ''
                        } ${!d.currentMonth ? 'bg-gray-50/40 text-gray-300' : 'bg-white text-gray-700'}`}
                      >
            ...
            <div className="mt-2 relative overflow-hidden rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 to-white p-5 flex justify-between items-center hover:shadow-xl hover:shadow-blue-100/70 transition-all duration-500 cursor-pointer">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-blue-100 flex items-center justify-center text-2xl shadow-3xs select-none">
                  🕒
                </div>
                <div>
                  <h3 className="font-bold text-[#1f2340]">
              ...
              <div className="space-y-3">
                {tasks.map((task) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className={`p-3 border border-gray-100 rounded-xl flex flex-col justify-between transition group relative gap-2 ${
                        task.isCompleted ? 'bg-gray-50/50 opacity-60' : 'bg-gray-50'
                      }`}
                    >
                      <div className="flex items-start gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <h4 className={`text-xs font-bold text-gray-800 truncate mt-0.5 ${task.isCompleted ? 'line-through text-gray-400' : ''}`}>{task.title}</h4>
                        </div>
                      </div>
                      <div className="flex items-center justify-between border-t border-gray-200/60 pt-1.5 text-[10px] text-gray-400">
                        <span className="font-medium">📅 {task.date}</span>
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleEditClick(task)} className="p-0.5 hover:bg-gray-200 rounded text-gray-500 hover:text-blue-600 text-[11px]">✏️</button>
                          <button onClick={() => handleDeleteTask(task.id)} className="p-0.5 hover:bg-gray-200 rounded text-gray-500 hover:text-red-600 text-[11px]">❌</button>
